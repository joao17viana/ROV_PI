...
def calcula_posicao_tanque(id, dist_x_to_marker, dist_y_to_marker, aux):
    ...
    if ids is not None:
        if len_n_ids == len(ids):
            ...
            if aux_mudanca == len(ids):
                ...
                aux_inicial = 0
                print("Apaguei")
                Pos_x, Pos_y, Pos_rot = limpa_arrays()
        ...
        if flag_estima_pos:
            ...
            for aux in range(len(ids)):
                Pos = estimaPos(frame, bbox[aux], ids[aux])
                print("Pos x", Pos[2] )
                print("Pos y", Pos[0] )
                
                Pos_Tanque_aux = calcula_posicao_tanque(ids[aux], Pos[2], Pos[0], aux)
                print("Pos x tanque", Pos_Tanque_aux[0])
                print("Pos x tanque", Pos_Tanque_aux[1])

                i = encontra_id(ids[aux][0])
            ...
            if (total_de_frames >= (N_frames_val - 1)):
                Pos_tanque_final = Validacao_pos(ids)
                aux_inicial_2 = 1
                Pos_x, Pos_y, Pos_rot = limpa_arrays()
                total_de_frames = 0
